...
(function($) {
  ...
  var defaults = {
    ...
    if (data) {
      $.each(data, function(index, value) {
        ...
        if ($.isPlainObject(value)) {
          if (settings.link) {
            str.push('<li data-icon=' + settings.icon + '><a href="'
              + settings.link + encodeURIComponent(value.value)
              + '" data-transition="' + settings.transition
              + '" data-autocomplete=\'' + JSON.stringify(value) + '\'>'
              + value.label + '</a></li>');
          } else {
            str.push('<li data-icon=' + settings.icon
              + '><a data-ajax="false" data-autocomplete=\'' + JSON.stringify(value) + '\'>'
              + value.label + '</a></li>');
          }
        } else {
          str.push('<li data-icon=' + settings.icon + '><a href="'
            + settings.link + encodeURIComponent(value) + '" data-transition="'
            + settings.transition + '">' + value + '</a></li>');
        }
      });
    }
    ...
    $target.html('').listview('refresh').closest("fieldset").removeClass(
      "ui-search-active");
    $this.trigger("targetCleared.autocomplete");
  }, timeout = null, timeoutClear = function() {
    if (timeout) {
    ...
    timeout = setTimeout(function() {
      console.log('qui');
      var id = $this.attr("id"), text, data, settings = $this
            ...
            $.ajax({
              ...
              beforeSend : function(jqXHR) {
                if (settings.cancelRequests) {
                  if (openXHR[id]) {
                    // If we have an open XML HTTP Request for this autoComplete
                    // ID, abort it
                    openXHR[id].abort();
                  } else {
                    // Set a loading indicator as a temporary stop-gap to the
                    // response time issue
                    settings.target.html(
                      '<li data-icon="none"><a href="#">Searching...</a></li>')
                      .listview('refresh');
                    settings.target.closest("fieldset").addClass(
                      "ui-search-active");
                  }
                  // Set this request to the open XML HTTP Request list for this
                  // ID
                  openXHR[id] = jqXHR;
                }
              },
    ...
    init : function(options) {
      ...
      .bind("keydown.autocomplete", timeoutClear)
      .bind("keyup.autocomplete", handleInput).next('.ui-input-clear').bind(
        'click', function(e) {
